...
zenith <- chicago_full %>% ggplot(aes(x=`Solar Zenith Angle`)) + geom_histogram(bins=360) + scale_x_continuous(breaks=seq(0,360,30)) + labs(title="Solar Zenith Angle Histogram", x = "Degrees", y = "Count")
...
ggplot(data = chicago_full, aes(x = Cloud_Type, y = GHI)) +
  ...
  labs(x = "Cloud_Type", y = "Mean Global Horizontal Irradiance (W/m²)",
       title = "Effect of Cloud Type on Irradiance \n (Solar Zenith: 0-80 degrees)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
